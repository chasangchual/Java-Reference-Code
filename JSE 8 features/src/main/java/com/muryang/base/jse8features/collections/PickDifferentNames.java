...
public class PickDifferentNames {
    ...
    public void run1() {
        Long countOfFriends = friends.stream().filter(startWithLetter.apply("N")).count() ;
        Long countOfEditors = editors.stream().filter(startWithLetter.apply("N")).count() ;
        Long countOfComrades = comrades.stream().filter(startWithLetter.apply("N")).count() ;
