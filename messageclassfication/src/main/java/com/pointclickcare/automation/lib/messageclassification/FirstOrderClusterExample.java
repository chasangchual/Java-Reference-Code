...
// 																									eval, 0.5);
		Clusterer clusterer = new GreedyAgglomerativeByDensity(training.getInstances().getPipe(), 
				eval, 0.5, false, new java.util.Random(1));
	...
	private Clustering sampleClustering (Alphabet alph) {
		InstanceList instances =
			new InstanceList(random, alph, new String[]{"foo", "bar"}, 30).subList(0, 20);
		
		InstanceList instancesAll =
				new InstanceList(random, alph, new String[]{"foo", "bar"}, 30);
		
		for (int i = 0; i < instancesAll.size(); i++) {
			Instance instance = instancesAll.get(i) ;
			System.out.println("instance ["+ String.valueOf(i) + "]") ;
			System.out.println("Source : " + instance.getSource()) ;
			System.out.println("Name : " + instance.getName()) ;
			System.out.println("Labeling : " + instance.getLabeling()) ;
			System.out.println("Data : " + (FeatureVector)(instance.getData())) ;
			
			FeatureVector v = (FeatureVector)(instance.getData()) ;
			v.print() ;
			// System.out.println("DataAlphabet : " + instance.getDataAlphabet()) ;
			...
			for (int j = i + 1; j < instances.size(); j++) {
				...
				if (fvi.contains("feature0") && fvj.contains("feature0")) {
					singletons = ClusterUtils.mergeClusters(singletons, singletons.getLabel(i),	singletons.getLabel(j));
									 ...
									 && random.nextUniform() < noise) {	// Random noise.
					singletons = ClusterUtils.mergeClusters(singletons, singletons.getLabel(i), singletons.getLabel(j));					
				}
			}
		}
